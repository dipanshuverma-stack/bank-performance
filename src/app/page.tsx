...
export default function Home() {
  ...
  // Sync Global Metrics with Cloud
  const metrics = userId && cloudMocks ? {
    syllabusMastery: localMetrics.syllabusMastery, // Syllabus remains local-first for now
    avgAccuracy: cloudMocks.length > 0 ? Math.round(cloudMocks.reduce((acc, m) => acc + (m.accuracy || 0), 0) / cloudMocks.length) : 0,
    mocksCount: cloudMocks.length,
    daysLeft: localMetrics.daysLeft
  } : localMetrics;
      ...
      <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 px-2">
        <div className="space-y-3 max-w-3xl min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
              ...
              <div className={`flex items-center gap-1.5 text-[9px] font-bold px-3 py-1 rounded-full border backdrop-blur-xl ${userId ? 'text-emerald-500 bg-emerald-500/5 border-emerald-500/20' : 'text-orange-500 bg-orange-500/5 border-orange-500/20'}`}>
                <Wifi className={`w-3.5 h-3.5 ${userId && 'animate-pulse'}`} />
                {userId ? "Cloud Sync Active" : "Local Mode"}
              </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8 min-w-0">
          ...
          <div className="p-6 rounded-[2rem] bg-card border border-border/60 hover:border-primary/40 transition-all duration-700 group shadow-xl">
             ...
             <Link href="/mocks" className="block w-full">
              <button className="w-full h-11 rounded-xl bg-accent/50 hover:bg-primary hover:text-primary-foreground transition-all text-[8px] font-black uppercase tracking-[0.15em] flex items-center justify-center gap-2 group/btn border border-transparent">
